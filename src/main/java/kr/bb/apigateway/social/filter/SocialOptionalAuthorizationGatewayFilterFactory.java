...
@Component
public class SocialOptionalAuthorizationGatewayFilterFactory extends
    AbstractGatewayFilterFactory<SocialOptionalAuthorizationGatewayFilterFactory.NameConfig> {


  public SocialOptionalAuthorizationGatewayFilterFactory() {
  }
  ...
  public GatewayFilter apply(NameConfig config) {
    return (exchange, chain) -> {
      if (!ExtractAuthorizationTokenUtil.isThereToken(exchange.getRequest())) {
    ...
    private Mono<Void> handleWhenThereIsToken(ServerWebExchange exchange, GatewayFilterChain chain)
    {
      if (!isAuthorizedUser(exchange)) {
        handleUnauthenticatedUser(exchange);
      }
      return chain.filter(exchange);
    }
